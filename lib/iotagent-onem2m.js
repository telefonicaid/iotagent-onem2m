...
function updateContextHandler(id, type, attributes, callback) {
    ...
    function updateAttribute(device, attribute, innerCallback) {
        var attributeName = attribute.name + generateRandomSeed();
        logger.debug(context, 'Updating attribute [%s] in device [%s]', attributeName, device.id);

        resourceService.createText(
            getAEName(device.service, device.subservice),
            device.id,
            attributeName,
            attribute.value,
            innerCallback);
