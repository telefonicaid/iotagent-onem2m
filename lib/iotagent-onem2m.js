...
function checkAndCreateContainer(aeName, attributeName, callback) {
    containerService.get(aeName, attributeName, function(error, result) {
        if (error) {
            containerService.create(aeName, attributeName, callback);
        } else {
            callback(null, result);
        }
    });
...
function provisioningHandler(device, callback) {
    ...
    function generateProcessHandler(attr, innerCallback) {
        return function(error, results) {
            if (error) {
                innerCallback(error);
            } else if (results && results.length > 0 && results[results.length - 1] && results[results.length - 1].ri) {
                attr.id = results[results.length - 1].ri;
                innerCallback(null, device);
            } else {
                innerCallback(new Error('Unknown error processing provisioning request.'));
            }
        };
    }
    ...
    function processActiveAttribute(attr, innerCallback) {
        async.series([
            apply(checkAndCreateContainer, aeName, attr.name),
    ...
    async.series([
        ...
        if (error) {
            callback(error);
        } else if (result && result[0] && result[0].ri) {
            device.internalId = result[0].ri;

            callback(null, device);
        } else {
            callback(new Error('Unknown error finding or creating AE.'));
        }
    });
}
...
function notificationHandler(notification, callback) {
    ...
    iotAgentLib.getDevicesByAttribute('internalId', notification.cr, function(error, devices) {
        if (error) {
            callback(error);
